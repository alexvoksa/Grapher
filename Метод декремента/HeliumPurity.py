...
Y = []
L = []
XYZ = float(1)  # в этой переменной записано наибольшее значение СКО интервала для декремента затухания
inter = int(2)  # в этой переменной записан шаг с которым мы проходим по общему диапазону
minscore = 0.01  # в этой переменной записано наименьшее допустимое значение к-та R^2
lis = [
...
def createdata():
    ...
    for n in names:
        ...
        # выбираем элементы у которых сумма столбца СКО минимальная для СКО меньше XYZ процента o = np.where((
        # finalframe.loc[where[1::3], 1:].sum(axis=0) / finalframe.loc[where[0::3], 1:].sum(axis=0)) < XYZ)[0].tolist()
...
def getgraph():
    ...
    plt.legend()
    plt.title('График зависимости декремента от частоты для различных ТВЭЛов')
...
createfinalframe()
print(finalframe)
#finalframe.to_csv(r'finalframe_TVEL_100_400_{}.csv'.format(inter), header=False, index=False)
#data0.to_csv(r'data0_TVEL_100_400_{}.csv'.format(inter), header=False, index=False)
...
#data0.to_csv(r'dataTVEL_0_400_{}.csv'.format(inter), header=False, index=False)
getgraph()
polypurity()
print(L)
...
print(L[wmax])
gdef()
print('В диапазоне ' + str(L[wmax][2] - inter) + '-' + str(L[wmax][2]) + 'кГц достигается наивысшая чувствительность'
                                                                     '. к-ты прямой y = k*x+b : ' + str(
    L[int(wmax)]) + 'СКО не превышает ' + str(XYZ * 100) + '%')
